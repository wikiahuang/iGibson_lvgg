...
def init_states(env, scenes_dir, scene_id, traj_id):
    ...
    return history_rgb, history_depth, history_pose, history_global_position, relative_point_goal, current_position, point_goal
...
def eval_scene(scene_id, eval_config):
    ...
    for traj_id in trajs[:30]:
        ...
        history_rgb, history_depth, history_pose, history_global_position, relative_point_goal, current_position, point_goal = init_states(
            env, scenes_dir, scene_id, traj_id
        )

        while np.linalg.norm(current_position - point_goal) > 1 and steps < 500:
            with torch.no_grad():
                actions = policy.inference(history_rgb, history_depth, history_pose, floorplan, None, relative_point_goal, None)
            ...
            relative_point_goal = torch.tensor(relative_point_goal).unsqueeze(0).to("cuda:0").float()
...
if __name__ == "__main__":
    ...
    eval_config = {
        ...
        "traj_save_dir": "/home/weiqi/data/iGibson/igibson/examples/scripts/results/eval_floverse_point_without_refiner",
    }
