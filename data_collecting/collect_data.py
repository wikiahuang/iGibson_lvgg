...
def path_directions(path_xy: np.ndarray, goal_directions=None) -> List:
    ...
    for i in range(n):
        if i == 0 and n > 1:
            v = path_xy[1] - path_xy[0]
        elif i > 0:
            v = path_xy[i] - path_xy[i - 1]
        else:
            v = np.array([1.0, 0.0], dtype=np.float32)
        norm = np.linalg.norm(v)
        dirs[i] = (v / norm) if norm > 1e-8 else np.array([1.0, 0.0], dtype=np.float32)
        ...
        if goal_dir_norm > 1e-8:
            ...
            if angle_diff > 5.0:
                ...
                if interpolated_positions:
                    path_xy = np.vstack([path_xy, np.array(interpolated_positions)])
                    dirs = np.vstack([dirs, np.array(interpolated_directions)])
...
def render_traj(sim: Simulator, 
    ...
    def _save_one(p, rgb_array, depth_array, rgb_dir, depth_dir):
        ...
        #     os.path.join(rgb_dir, f"rgb_{p}.png"),
        #     compress_level=1,  # 关键：降低PNG压缩等级
        #     optimize=False
        # )
        cv.imwrite(os.path.join(rgb_dir, f"rgb_{p}.png"), rgb_uint8,
        ...
        #     os.path.join(depth_dir, f"depth_{p}.png"),
        #     compress_level=1,
        #     optimize=False
        # )
...
""")
    ...
    if is_single_scene_input:
        ...
        scenes = [scene_name]
        dataset_root = parent_dir  # 场景实际所在的父目录
        
        # print("\n" + "="*70)
        # print("Mode: Single Scene")
        # print("="*70)
        # print(f"Scene path   : {input_path}")
    ...
    # print(f"Trajectories : {args.num_trajectories} per floor")
    # print(f"Min length   : {args.min_distance} m")
    # print(f"Headless     : {args.headless}")
    # print(f"iGibson data : {igibson.ig_dataset_path}")
    # print("="*70 + "\n")
    ...
    for i, scn in enumerate(scenes, 1):
        # print("\n" + "="*70)
        # print(f"[{i}/{len(scenes)}] {scn}")
        # print("="*70)
        try:
            scene_a_atart = time.time()
            # ===== 修改点 3: 使用 dataset_root 而不是 args.dataset_path =====
